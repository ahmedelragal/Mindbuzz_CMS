...
class ReportController extends Controller
{
    ...
    public function classContentEngagementReport(Request $request)
    {
        ...
        // dd($games->lesson->unit->program->course);

        $games = Game::with(['lesson.unit.program.course'])
            ->whereHas('lesson.unit.program.course', function ($query) {
                $query->where('id', 1);
            })->whereHas('lesson.unit.program', function ($query) {
                $query->where('id', 7);
            })
            ->limit(5)
            ->get();
        ...
        // $stageNames = $games->map(function ($game) {
        //     return $game->lesson->unit->program->stage->name;
        // });
